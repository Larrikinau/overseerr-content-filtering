...
- ✅ **Person Pages** - Cast/crew now properly filtered by certification (R-rated/NR content blocked for restricted users)
- ✅ **Collection Pages** - Collection parts now filtered by user rating restrictions
- ✅ **Series View** - Added server-side TV rating backup filter (fixes TV-14 breakthrough for TV-PG users)
- ✅ **Trending Page** - Restored vanilla behavior + added curated filter support (0/0 settings now work correctly)
- ✅ **Post-Filtering Architecture** - All endpoints now use type-safe post-filtering for reliable certification enforcement
