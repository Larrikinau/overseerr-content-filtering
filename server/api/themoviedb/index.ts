    ...
    for (const movie of movies) {
      try {
        ...
        if (usCertification && usCertification.release_dates?.length > 0) {
          // Find the first release with certification data
          const releaseWithCert = usCertification.release_dates.find(
            (release: any) => release.certification && release.certification.trim() !== ''
          );
          
          if (releaseWithCert) {
            const movieCert = releaseWithCert.certification.trim();
            const movieRatingOrder = ratingOrder[movieCert] || 999;
            
            // Include movie only if its rating is within the allowed range
            if (movieRatingOrder <= maxRatingOrder) {
    ...
    for (const tvShow of tvShows) {
      try {
        ...
        if (usContentRating && usContentRating.rating && usContentRating.rating.trim() !== '') {
          const tvRating = usContentRating.rating.trim();
          const tvRatingOrder = ratingOrder[tvRating] || 999;
          
          // Include TV show only if its rating is within the allowed range
          if (tvRatingOrder <= maxRatingOrder) {
