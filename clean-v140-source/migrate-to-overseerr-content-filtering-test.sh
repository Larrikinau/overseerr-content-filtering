...
# Universal Overseerr Content Filtering Migration Script v1.4.0 - TEST VERSION
# This version uses the locally built image for testing
# This script handles:
...
# Configuration - Using local built image for testing
CONTAINER_NAME="overseerr"
IMAGE_NAME="overseerr-content-filtering"
TAG="v1.4.0"
...
detect_installation() {
    ...
    if [ -f "$COMPOSE_FILE" ]; then
        if grep -q "larrikinau/overseerr-content-filtering\|overseerr-content-filtering" "$COMPOSE_FILE" 2>/dev/null; then
            ...
            if echo "$image" | grep -q "larrikinau/overseerr-content-filtering\|overseerr-content-filtering"; then
                installation_type="fork"
...
start_services() {
    ...
    # Skip pulling since we're using locally built image for testing
    log_info "Using locally built image: ${IMAGE_NAME}:${TAG}"
...
main() {
    echo ""
    echo "🎬 Overseerr Content Filtering Migration Script v1.4.0 - TEST VERSION"
    echo "====================================================================="
    echo ""
